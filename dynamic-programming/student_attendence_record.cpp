...
public:
    static int check_all_records(int cur_ind, int count_a, int count_l, int n, vector<vector<vector<int>>>& temp) {
        if (cur_ind == n) {
            return 1;
        }
...
int main() {
    vector<vector<vector<int>>> temp(2, vector<vector<int>>(2, vector<int>(3, -1)));
    cout << Solution::check_all_records(0, 0, 0, 2, temp) << endl;
}